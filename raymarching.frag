...
float map(vec3 pos) {
    ...
    world *= boxSDF(pos, vec3(0.5, 0.7, 1.));
...
void main(){
    ...
    vec2 mouse_uv = (2.0 * u_mouse - u_resolution.xy) / u_resolution.y;
    ...
    for(int i = 0; i < steps; ++i) {
        ...
        float scaling = 1.;
        float dist = map(transform(vec3(sin(u_time), vec2(0)), scaling, pos)) * scaling; // Current distance to the scene
